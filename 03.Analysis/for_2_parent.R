selectSample_and_filterGT <- function(data, highP, lowP, highB, lowB, minGQ){
  ...
  dd1 <- data %>%
    select(CHROM, POS, REF, ALT,
           ...
           all_of(c(paste(lowP, "GT", sep = "."), paste(lowP, "AD", sep = "."), paste(lowP, "GQ", sep = "."))),
  ...
  dd2 <- dd1 %>% filter(highParent.GQ >= minGQ, lowParent.GQ >= minGQ) %>%
  ...
  dd3 <- dd2 %>%
    ...
    mutate(highParent.DP = highParent.AD_0 + highParent.AD_1,
           lowParent.DP = lowParent.AD_0 + lowParent.AD_1,
  ...
  dd5 <- dd4 %>% mutate(HB.HP.AD = if_else(highParent.GT == paste(REF, REF, sep = "/"),
    ...
    dplyr::select(CHROM, POS, REF, ALT, HP.DP = highParent.DP, LP.DP = lowParent.DP,
...
depth_statistics <- function(data, outPrefix, highP, lowP, highB, lowB){
  ...
  ggsave(P_dp, filename = paste(outPrefix, "depth_density.pdf", sep = "."), height = 3.5, width = 10)
  ggsave(P_dp, filename = paste(outPrefix, "depth_density.png", sep = "."), height = 3.5, width = 10, dpi = 500)
}
