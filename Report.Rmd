---
title: "茄子（*Solanum melongena* L.）QTL-seq分析"
...
aligner <- "bwa-mem2"  # 
genome <- "茄子HQ-1315"
genome_ref <- "Wei, Q., Wang, J., Wang, W., Hu, T., Hu, H., & Bao, C. (2020). A high-quality chromosome-level genome assembly reveals genetics for important traits in eggplant. Horticulture research, 7(1), 153. https://doi.org/10.1038/s41438-020-00391-0"
genome_url <- "http://www.eggplant-hq.cn/Eggplant/home/index"
...
if (aligner == "bwotie2") {
  ...
  mapInfo <- dataInfo %>% select(Sample = sample, `Total reads` = total_reads.clean) %>% 
    ...
    select(Sample, `Total reads`, `Mapped reads`, `Mapped rate`, `Uniquely mapped read`, `Uniquely mapped rate`)
}
...
file.copy(from = paste("./02.SNP_indel/", prefix, ".flt.report.html"), to = paste("./result/02.SNP_indel/", prefix, ".report.html", sep = ""))
```
