...
library(tidyverse)
cfg <- jsonlite::fromJSON("./script/config.json")
prefix <- cfg$call_vari$filename
aligner <- cfg$call_vari$aligner  # 
if (aligner == "mem") {
  ...
  alignr_ref <- "Dobin, A., Davis, C. A., Schlesinger, F., Drenkow, J., Zaleski, C., Jha, S., Batut, P., Chaisson, M., & Gingeras, T. R. (2013). STAR: ultrafast universal RNA-seq aligner. Bioinformatics (Oxford, England), 29(1), 15–21. https://doi.org/10.1093/bioinformatics/bts635"
}
genome <- "苦瓜MC.update"
genome_ref <- "NULL"
genome_url <- "NULL"
...
if (aligner == "bwotie2" || aligner == "star") {
  mapInfo <- read_csv(file = "./01.Mapping/align_stat.csv")
} else if (aligner == "mem" || aligner == "mem2") {
  mapInfo <- read_csv(file = "./01.Mapping/align_stat.csv")
...
# QTL-seq原理
QTL-seq[^1]是一种将Bulked‐segregant analysis (BSA)[^2] [^3]和高通量测序相结合，快速定位QTL的方法。目标性状有差异的双亲构建的分离群体中分别选取极端表型个体进行等量混合构建两个极端表型bulked DNA（或RNA） pool并进行测序。随后进行变异分析筛选出双亲间SNP位点并分别计算两个bulked pool中每个SNP位点上某一亲本基因型read覆盖深度占该位点总read深度的比值，即SNP index，通过两个bulked pool的SNP index相减即得到ΔSNP index。在基因组所有区域中，目标基因及其连锁的区域由于根据表型受到相反的选择在两个bulked pool中表现出不同的趋势，因此ΔSNP index会显著偏离0附近；另一方面，于目标形状无关的区域则两个bulked pool则表现为相似的变化趋势，因此ΔSNP index会在0附近波动。
...
## 取样、提取DNA、建库和测序
选取分离群体中极端表型个体各30-50株以及双亲取幼嫩组织，分别提取DNA（或RNA）并将分离群体极端表型个体按照等量原则混合构建bulked pools，然后进行建库和高通量测序 (混池DNA深度一般与单株个数相同) (具体流程应参考实验设计以及公司测序报告)。
...
## 比对到参考基因组
```{R include=FALSE}
alg_tool <- switch (aligner,
  star    = "STAR[^5]（version：2.7.3a）",
  mem     = "bwa-mem[^5]（version：0.7.17-r1188）",
  mem2    = "bwa-mem2[^5]（version：2.2.1）",
  bowtie2 = "Bowtie2[^5]（version：2.4.5）",
  stop("未知 aligner")
)
sort_tool <- switch (cfg$call_vari$sort,
  sbb = "sambamba[^8]（version：0.8.2）",
  sts = "samtools[^7]（version：1.15.1）",
  stop("未知 sort")
)
rmdup_tool <- switch (cfg$call_vari$rmdup,
  sbb    = "sambamba[^8]（version：0.8.2）",
  picard = "Picard（version：2.23.3）",
  stop("未知 rmdup")
)
```
```{r, results='asis'}
if (aligner == "mem" || aligner == "mem2" || aligner == "bowie2") {
  cat("使用", alg_tool, "软件将clean read比对到", genome, "参考基因组[^6]，得到SAM (Sequence Alignment/Map) 格式文件[^7]并对比对结果进行统计 (./result/01.Mapping/align_stat.csv)，随后使用", sort_tool, "软件将比对结果（SAM文件）转换为BAM（Binary Alignment/Map）格式文件[^7]以及按照染色体位置排序，并使用", rmdup_tool, "软件去除建库过程中产生的PCR重复，然后统计样本基因组覆盖率 (./result/01.Mapping/cov_stat.csv)。\n", sep = "")
} else if (aligner == "star") {
  cat("使用", alg_tool, "软件将clean read比对到", genome, "参考基因组[^6]，得到排序后的BAM (Binary Alignment/Map) 格式文件[^7]并对比对结果进行统计 (./result/01.Mapping/align_stat.csv)，随后使用", rmdup_tool, "软件去除建库过程中产生的PCR重复，并统计样本基因组覆盖率（./result/01.Mapping/cov_stat.csv），使用Genome Analysis Toolkit，GATK[^9]（version: 3.8-0-ge9d806836）中的SplitNCigarReads用来处理比对中的跨外显子（spliced）reads，把它们拆分成可以被GATK正确识别的形式。\n", sep = "")
}
```
...
[^8]: Tarasov, A., Vilella, A. J., Cuppen, E., Nijman, I. J., & Prins, P. (2015). Sambamba: fast processing of NGS alignment formats. Bioinformatics (Oxford, England), 31(12), 2032–2034. https://doi.org/10.1093/bioinformatics/btv098
