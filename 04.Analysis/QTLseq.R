...
p <- add_argument(p, "--maxLPdp", help = "Maxmum depth for low parent", type = "numeric", default = 150)
...
p <- add_argument(p, "--winStep", help = "Window step for sliding window statistics", type = "numeric", default = 100000)
#
...
if (FALSE) {
  ## file name
  filename <- "./BSA.filter.INDELs.table.gz"
  outPrefix <- "133"
  
  CI <- "./QTLseqCI.indH33_indL34_F2.Depth_5_150.Rep_10000.RData"
  ## filter SNP Quality
  minQ <- 30
  ##
  bulkSizeH <- 33
...
outtb <- dd6 %>% 
  dplyr::select(CHROM, POS, HB.HP.AD, HB.LP.AD, HB.DP, LB.HP.AD, LB.LP.AD, LB.DP)
colnames(outtb) <- c("CHROM", "POS", 
                     paste(highB, highP, "depth", sep = "."),
                     paste(highB, lowP, "depth", sep = "."),
                     paste(highB, "total_depth", sep = "."),
                     paste(lowB, highB, "depth", sep = "."),
                     paste(lowB, lowP, "depth", sep = "."),
                     paste(lowB, "total_depth", sep = "."))
